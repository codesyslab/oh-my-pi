...
- MCP server allowlist: agent frontmatter `mcpServers` narrows the inherited parent MCP tool surface (omitted / `"*"` preserves the parent's full set; `[]` exposes none; a non-empty list exposes only tools whose raw `mcpServerName` is in the list). `task.agentMcpServers[agentName]` is the harness-level override (agent name → `"*"` or exact server list, including `[]`) that beats frontmatter; an absent entry falls back to frontmatter, both absent preserves all. Persisted on `session_init` and re-applied on cold revival so a parked subagent cannot regain excluded tools through inherited manager discovery or `alwaysInclude` behavior. See `task-agent-discovery.md` for the parser contract.
